...
import numpy as np
from tqdm import tqdm # Tambahkan tqdm untuk progress bar (opsional, tapi disarankan)
...
# ======================= TRANSFORMS =======================
# Transformasi untuk Training (dengan Data Augmentation)
...
# Transformasi untuk Validasi (tanpa Data Augmentation)
...
def train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes,
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        for phase in ["train", "val"]:
            ...
            # Tambahkan tqdm untuk melihat progress di setiap phase
...
if __name__ == "__main__":
    ...
    # 1. Ambil class_to_idx dari folder data/train
    ...
    # 3. Buat Dataset dan Dataloader dari folder TRAIN dan VAL
    ...
    # 4. Hitung Class Weight secara EFISIEN
    print("⏳ Menghitung Class Weights (Ini mungkin memakan waktu)...")
    
    # DataLoader sementara tanpa shuffle dan batch size besar untuk mengambil semua label
    temp_loader = DataLoader(full_train_dataset, batch_size=2048, shuffle=False, num_workers=NUM_WORKERS)
    
    labels_list = []
    # Iterasi melalui data loader untuk mengambil label
    ...
    # Pad class counts dengan nol jika ada kelas yang tidak ada di data train
    ...
    # 5. Inisialisasi Dataloaders utama
    dataloaders = {
    ...
    if os.path.exists(CHECKPOINT_PATH):
        ...
        is_unfrozen = checkpoint.get("is_unfrozen", False)

        if is_unfrozen:
             print("🔧 Optimizer di-re-initialize untuk fine-tuning seluruh model...")
             optimizer = torch.optim.Adam(model.parameters(), lr=1e-5)
             scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=7, gamma=0.1)

        print(f"✅ Melanjutkan dari epoch {start_epoch} (best acc {best_acc:.4f})")
